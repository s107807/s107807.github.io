...
pygame.display.set_caption("Vemulate Brain")
...
class screen():
	...
	def screenScaling(scale):
		whileLoopAntiCrash()
		print('unfinished dont use without knowing how to use')
	...
	def print(text):
		...
		surface.blit(f.render(text,True,penColor),(int((screenReslution[1] * screenScale)/40)*(int(screenColumn)-1), int((screenReslution[0] * screenScale)/12)*(int(screenRow)-1)))
		pygame.display.flip()
	...
	def clear_screen(color = fillColor):
		whileLoopAntiCrash()
	...
	def draw_circle(x = 0, y = 0, radius = 10, color = fillColor):
		whileLoopAntiCrash()
		global surface, screenRow, screenColumn, screenReslution, screenScale, fillColor, penColor, penWidth
		pygame.draw.circle(surface, penColor, ((x * screenScale), (y * screenScale)), (radius * screenScale))
		pygame.draw.circle(surface, color, ((x * screenScale), (y * screenScale)), (radius * screenScale) - (int(penWidth/10) * screenScale))
		pygame.display.flip()
